 ...
 */
export function submitCheckResults(results: CheckResult[]): Promise<CheckResult[]> {
  return new Promise((resolve, reject) => setTimeout(() => (Math.random() <= 0.8 ? resolve(results) : reject({ success: false })), 500));
}
